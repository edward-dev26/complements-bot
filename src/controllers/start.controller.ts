...
export class StartController implements IController {
  ...
  execute(context: IControllerContext, provider: IProvider) {
    ...
    const message = `Любимая, Инночка, ты будушь получать комплименты каждый день в 10 утра. Но ты можешь не ждать этого времени и получуть комплимент прямо сейчас выполнив /complement команду.
